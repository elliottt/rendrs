...
impl Obj {
    ...
    pub fn new() -> Self {
        Obj{ vertices: Vec::new(), faces: Vec::new() }
    }
    ...
    pub fn parse<In>(input: In) -> Result<Self,Error> where In: BufRead {
        let mut obj = Obj::new();
        ...
        for line in input.lines() {
            ...
            match chunks.next() {
                Some("v") => {
                    let a = chunks.next().ok_or_else(|| format_err!("bad vertex"))?.parse()?;
                    let b = chunks.next().ok_or_else(|| format_err!("bad vertex"))?.parse()?;
                    let c = chunks.next().ok_or_else(|| format_err!("bad vertex"))?.parse()?;
                    obj.push_vertex(Point3::new(a, b, c));
                },

                Some("f") => {
                    let a = chunks.next().ok_or_else(|| format_err!("bad face"))?.parse()?;
                    let b = chunks.next().ok_or_else(|| format_err!("bad face"))?.parse()?;
                    let c = chunks.next().ok_or_else(|| format_err!("bad face"))?.parse()?;
                    obj.push_face([a, b, c]);
                },
